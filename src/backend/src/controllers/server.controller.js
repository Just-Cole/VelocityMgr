...
class ServerController {
    ...
    async _addServerToProxyConfig(serverToAdd, proxyServer) {
        try {
            ...
            parsedToml.forwarding['player-info-forwarding-mode'] = 'modern';

            if (!parsedToml.try) parsedToml.try = [];
            
            // Ensure the new server is at the front of the try list.
            const otherServers = parsedToml.try.filter(s => s !== serverEntryName);
            parsedToml.try = [serverEntryName, ...otherServers];
            ...
            console.log(`[Link Server] Successfully added/updated server '${serverToAdd.name}' in proxy '${proxyServer.name}' config.`);
        } catch (tomlError) {
        ...
        try {
            ...
            await fsPromises.writeFile(path.join(serverFolderPath, 'eula.txt'), 'eula=true', 'utf-8');
             ...
             if (isPaper) {
                ...
                // Use a predefined template for velocity.toml
                const baseTemplate = `
config-version = "2.7"
...
"Hub.example.com" = [ "Hub" ]
                    ...
                    serversSection = `
[servers]
# No companion server created. Add backend servers here.
# Example:
# try = [ "lobby" ]
