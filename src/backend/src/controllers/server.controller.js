...
const velocityTomlTemplate = `config-version = "2.7"
bind = "0.0.0.0:25565"
            ...
            if (newServer.softwareType === 'PaperMC') {
                ...
                if (!fs.existsSync(tomlPath)) {
                    ...
                    const finalTomlContent = velocityTomlTemplate
                        ...
                        .replace(/port\s*=\s*25_?565/, `port = ${newServer.port}`);
            ...
            if (newServer.softwareType === 'PaperMC') {
                ...
                if (proxyServer) {
                    try {
                        ...
                        let tomlLines = [];
                        if (fs.existsSync(tomlPath)) {
                            tomlLines = (await fsPromises.readFile(tomlPath, 'utf8')).split(/\r?\n/);
                        } else {
                            // This shouldn't happen if a proxy exists, but as a fallback
                            tomlLines = velocityTomlTemplate.split(/\r?\n/);
                        }
                        ...
                        // Helper to insert lines into sections
                        const insertIntoSection = (lines, sectionName, lineToInsert) => {
                            ...
                            if (sectionIndex !== -1) {
                                ...
                                // Insert before the end of section or at the end of file
                        ...
                        // Update the 'try' array if it's empty
