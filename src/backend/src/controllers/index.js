...
class IndexController {
    ...
    async _syncVelocitySecret(paperServer, velocityProxy) {
        ...
        if (!paperServerPath || !velocityProxyPath) {
            console.error(`[Secret Sync] Could not determine folder path for one of the servers.`);
            return;
        }
    
        const secretFilePath = path.join(velocityProxyPath, 'forwarding.secret');
        if (!fs.existsSync(secretFilePath)) {
            console.error(`[Secret Sync] forwarding.secret not found for proxy ${velocityProxy.name}. Cannot sync secret.`);
            return;
        }
    
        try {
            const secret = (await fsPromises.readFile(secretFilePath, 'utf-8')).trim();
            ...
            // This is a correct and functional configuration for paper-global.yml with Velocity.
            const yamlLines = [
                ...
                '    enabled: true',
                '    online-mode: true',
                `    secret: '${secret}'`, // Using single quotes for safety
                '  bungee-cord:',
                '    online-mode: true',
                'settings:',
                '  # This is for advanced setups. Do not change unless you know what you are doing.',
            ...
            console.log(`[Secret Sync] Successfully synced forwarding secret from ${velocityProxy.name} to ${paperServer.name}.`);
